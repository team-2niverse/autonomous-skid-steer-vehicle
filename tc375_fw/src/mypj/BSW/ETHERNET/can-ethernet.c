#include <ETHERNET/can-ethernet.h>
#include "lwip/opt.h"
            ...
            if (p != NULL) {
                ...
                while (q != NULL) {
                    ...
                    for (uint16_t i = 0; i < q->len; i++) {
                        canData[canIndex++] = data[i];

                        if (canIndex == 8) {
                            Can_Send_Msg(0x100, canData, 8);
                            canIndex = 0;
                        }
                    }
                    q = q->next;
                ...
                // 남은 데이터 처리
                if (canIndex > 0) {
                    Can_Send_Msg(0x100, canData, canIndex);
                }
            }
            tcp_sent(tpcb, can_eth_sent);
        } else {
...
//		Set_CAN_msgObject(0, 0, i, dlc, 0);
        Can_Send_Msg(0, ((char*) (p->payload) + (i * 8)), dlc);
        delay_ms(100);
    }
}
...
//			Set_CAN_msgObject(0, 0, 0x200, dlc, 0);
            Can_Send_Msg(0x200, (char*) pdu_data + j * 8, dlc);
