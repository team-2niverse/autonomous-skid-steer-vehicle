...
void TcpEchoInit(void)
{
    ...
    {
        err_t err = tcp_bind(g_tcpEchoPcb, IP_ADDR_ANY, 1000); /* ...bind the TCP procotol control block to any local address and local port 1000. */
        ...
        {
            LWIP_DEBUGF(ECHO_DEBUG | LWIP_DBG_STATE, ("Echo: unable to bind to any address on port 1000.\n"));
        }
    }
    else
    {
