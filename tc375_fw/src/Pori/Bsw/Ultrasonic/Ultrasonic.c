...
void echo_initInt(void);
void Stm0_initInt10us(void);
...
float Ultrasonic_getBackDist(void) {
    return dist_back;
}

float Ultrasonic_getLeftDist(void) {
    return dist_left;
}

float Ultrasonic_getRightDist(void) {
    return dist_right;
}

void Ultrasonic_initInt(void) {
    // Back
...
IFX_INTERRUPT(IsrHandler_stm0, 0, ISR_PRIORITY_STM0);
IFX_INTERRUPT(IsrHandler_eru2, 0, ISR_PRIORITY_ERU2);

void IsrHandler_stm0(void) {
    MODULE_STM0.CMP[0].U = (unsigned int)((MODULE_STM0.TIM0.U | ((uint64)MODULE_STM0.CAP.U << 32)) + 10*CPU_CLOCK_MHZ);
    stm0_cnt++;
    if (stm0_cnt == 1) {
...
void IsrHandler_eru2(void) {
    if (MODULE_SCU.EIFR.B.INTF3) {
        if (MODULE_P02.IN.B.P0)
            start_back = Stm0_getTimeUs();
        else
            dist_back = (float)0.0343 * (Stm0_getTimeUs() - start_back) / 2.0;
        MODULE_SCU.FMR.B.FC3 = 1;
    } else if (MODULE_SCU.EIFR.B.INTF5) {
        if (MODULE_P15.IN.B.P8)
            start_left = Stm0_getTimeUs();
        else
            dist_left = (float)0.0343 * (Stm0_getTimeUs() - start_left) / 2.0;
        MODULE_SCU.FMR.B.FC5 = 1;
    } else if (MODULE_SCU.EIFR.B.INTF7) {
        if (MODULE_P15.IN.B.P1)
            start_right = Stm0_getTimeUs();
        else
            dist_right = (float)0.0343 * (Stm0_getTimeUs() - start_right) / 2.0;
